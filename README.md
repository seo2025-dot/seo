...
3. *(Opcional, recomendado para probar)* pega y ejecuta `supabase/seed.sql`: crea 23 perfiles demo, ofertas, búsquedas, servicios, vacantes y publicaciones. Los perfiles demo **no pueden iniciar sesión** y aceptan algunos likes en Citas para poder probar con una sola cuenta.
   ...
   > **¿Y la 010?** Ejecuta `supabase/update_011_admin_monedas.sql` (panel de administración de monedas en `/admin/monedas`: ventas, tarifas, paquetes, soporte y registro de auditoría; solo para administradores).
   > **¿Y la 011?** Ejecuta `supabase/update_012_geolocalizacion.sql` (geolocalización e internacionalización: país en perfiles y negocios, ubicación aproximada y privada, y búsqueda «cerca de mí» por país; ver [docs/geolocalizacion-y-dia.md](docs/geolocalizacion-y-dia.md)). Reemplaza `search_providers` (un parámetro más), así que aplícala antes de desplegar.
   > **¿Y la 012?** Ejecuta `supabase/update_013_genero_y_preferencias.sql` (a todas las personas se les pregunta si son hombre o mujer —o prefieren no decirlo— y a quién quieren conocer; las recomendaciones respetan ambos sentidos; ver [docs/geolocalizacion-y-dia.md](docs/geolocalizacion-y-dia.md)). `complete_onboarding` pasa a exigir esos dos datos, así que aplícala antes de desplegar el onboarding nuevo.
   > **¿Y la 013?** Ejecuta `supabase/update_014_admin_alertas.sql` (bandeja de avisos para el administrador —registro, negocio nuevo y verificación enviada—, panel `/admin` y cola de identidad con cédula, selfie y foto de perfil). Para recibir los correos configura `RESEND_API_KEY` y `ADMIN_ALERT_EMAIL` (ver [docs/avisos-y-administracion.md](docs/avisos-y-administracion.md)).
...
├── update_011_admin_monedas.sql       Panel de administración de monedas: estadísticas, ventas, ajustes, soporte y auditoría
├── update_012_geolocalizacion.sql     Geolocalización: país en perfiles y negocios, ubicación privada (~1 km) y búsqueda por país
├── update_013_genero_y_preferencias.sql  Género y a quién quiere conocer (privados), recomendaciones en ambos sentidos y people_i_may_meet
├── update_014_admin_alertas.sql       Avisos al administrador (cola), admin_overview y admin_kyc_queue
...
- **Reflexiones** (`src/lib/reflexiones.ts`): 90 mensajes de tres tradiciones (espiritismo de Allan Kardec, estoicismo y psicología) sobre autoconocimiento, serenidad, gratitud, resiliencia, vínculos y más. Cada persona ve una distinta cada día (rotación determinista: se recorre todo el catálogo antes de repetir, nunca dos días seguidos la misma y se alternan las tradiciones) y puede pedir «Otra reflexión». Solo se marcan como cita (`fuente`) las pocas frases textuales comprobables (Kardec, Séneca, Epicteto, Marco Aurelio, Frankl, Rogers); el resto se presenta como reflexión propia «inspirada en», sin atribuir palabras a nadie.
- **El día de hoy** (portada, `PulsoDelDia`, y reloj en la barra superior): la plataforma se siente viva con datos reales calculados en el dispositivo para el lugar de la persona: hora y fecha en su zona horaria, saludo y frase de entrada que cambian cada día y en cada visita, fase de la luna, amanecer y ocaso, signo solar, la próxima efeméride (feriados de Ecuador, Pascua, Día de la Madre y del Padre, fechas mundiales) y un tema de la semana con enlace a una acción. Sin cifras inventadas.
- **Internacional y personalizada**: la plataforma es solo en español pero abierta al mundo. Están **todos los países hispanohablantes** (21, primero en los selectores) y otros 28 (Europa, Américas, Asia…) para que cualquier persona pueda inscribirse. **Al inscribirse se pregunta a todas las personas si son hombre o mujer (o prefieren no decirlo) y a quién les gustaría conocer**: ambos datos son privados, `recommend_people` y la baraja de `/citas` solo muestran a quienes encajan **en los dos sentidos**, y los textos se conjugan en su género (`src/lib/genero.ts`). Quienes ya estaban inscritas ven un aviso para completarlo. **Pagos por país**: PayPhone solo en Ecuador; en el resto del mundo, PayPal (el servidor rechaza PayPhone a quien indicó otro país).
- **Sin «código» en pantalla** (`next.config.ts`, `src/app/{error,global-error,not-found}.tsx`, `docs/renderizado-y-errores.md`): las respuestas de datos de Next (RSC) y el HTML nunca se guardan en cachés compartidas (así ningún CDN o proxy puede servir «código» en vez de la página), hay pantallas de error y 404 en español con salidas claras, y si el navegador carga archivos de una versión vieja se recarga solo una vez.
- **Avisos y panel de administración** (`/admin`, `src/lib/avisos/**`): te llega un **correo** cuando alguien **se registra**, **registra un negocio** o **envía su cédula** para verificar (correo, fecha, rol inicial…). Los avisos pasan por una cola en la base de datos: si el correo falla o aún no está configurado, no se pierden. El **panel `/admin`** (botón «Admin» violeta en la barra superior, con el número de verificaciones pendientes) muestra lo que necesita atención, cuánta gente se registra y los últimos registros y negocios; **`/admin/kyc`** enseña la cédula, la selfie y la foto de perfil de cada persona lado a lado para aprobar o rechazar con un toque.
- **Geolocalización** (`src/lib/geo.ts`, `src/features/geo/**`): pensada para una super app internacional. 49 países y sus ciudades principales; la ubicación se deduce de la zona horaria del dispositivo y la persona puede cambiarla o usar el GPS (solo si lo pulsa). Privacidad: se guarda **aproximada (2 decimales ≈ 1 km)**, solo la ve su dueña o dueño (`user_private`); lo único público es el país. Los negocios indican país, ciudad y punto (~100 m) y en los directorios el botón **«📍 Cerca de mí»** ordena por distancia dentro del país (`search_providers` con `p_lat`, `p_lng`, `p_country`).
...
- **Monedas y tienda** `/monedas`: los **3 primeros usos de cada acción son gratis** (pedir, aceptar un pedido, pedir ofertas, ofertar, publicar un evento, reservar entradas; en las citas, 3 mensajes gratis por conversación) y desde el cuarto cuestan monedas. Se compran desde **$0.50** (paquetes de $0.50, $1.50 y $3.50, con +25 % en la primera compra) o se ganan con **retos** (diarios, semanales y únicos: invitar amigos, calificar comercios, pedir, reservar…). **Panel de administración** `/admin/monedas` (solo administradores): resumen de la economía, ventas, ajuste de tarifas, paquetes, bonificación y retos sin tocar SQL, soporte por persona (saldo, usos, pagos y ajustes con motivo) y registro de auditoría. Pagos con **PayPhone y PayPal** ya conectados: solo faltan las credenciales (ver [docs/monedas-y-pagos.md](docs/monedas-y-pagos.md)).

Probado: 93 pruebas de base de datos (`supabase/tests/directorios.test.mjs`, incluido el flujo real de alta y edición, y la paridad cliente↔servidor de pedidos, solicitudes, ofertas y eventos) y 68 de lógica de interfaz (`supabase/tests/directorio-ui.test.mjs`); además, 38 de la economía de monedas y su panel de administración contra PostgreSQL real (`monedas.test.mjs`) y 40 de las pasarelas de pago, la lógica del panel y el icono de moneda con PayPhone y PayPal simulados (`pagos.test.mjs`). Diseño completo (modelo, flujos, rutas y componentes, pautas de autoservicio, riesgos y fases): **[docs/arquitectura-directorios.md](docs/arquitectura-directorios.md)**.
